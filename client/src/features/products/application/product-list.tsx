...
import { ProductInterface, getProduct, getProductsBySection } from "..";
    ...
    const fetchProducts = async () => {
      try {
        if (section === "All") {
          ...
          const productData = await getProductsBySection(section);
          setProducts(productData);
        }
      } catch (error) {
  ...
  if (products.length === 0) {
    return (
      ...
      >
        ...
        >
          Parece que no hay productos disponibles en esta categoría.
        </p>
      </div>
    );
  }
